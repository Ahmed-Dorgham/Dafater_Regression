...
public class BanksListPage extends MainPage {
    private String dataMigrationTitle = "data migration";
    // private WebDriver driver ;

    public BanksListPage(WebDriver driver) {
        this.driver = driver;
    }
    ...
    private By bankLabel = By.xpath("(//h3[contains(text(),'البنك')])" +
            "| (//h5[contains(text(),'قائمة مصرف')])");
    private By totalBanks = By.xpath("(//*[contains(@class,'total-rows')])" +
...
//        Thread.sleep(20000);
    ...
    public String getNumberOfAllBanksBeforeSyncing() {

        waitUntilElementToBePresent(bankLabel, GeneralConstants.minTimeOut);
        System.out.println("number of banks at list view before Syncing " + getWebElement(totalBanks).getText());
        Allure.step("number of banks at list view before Syncing " + getWebElement(totalBanks).getText());
        return getWebElement(totalBanks).getText();
    }

    public String getNumberOfAllBanksAfterSyncing() throws InterruptedException {

        waitUntilElementToBePresent(bankLabel, GeneralConstants.minTimeOut);
        waitUntilOverlayDisappear(overlay, GeneralConstants.freezeTimeOut);
        waitUntilElementNotHaveSpecificText(listCount, "تحديث");
        System.out.println("number of banks at list view after Syncing " +getWebElement(totalBanks).getAttribute("textContent"));
        Allure.step("number of banks at list view after Syncing " + getWebElement(totalBanks).getAttribute("textContent"));
        return getWebElement(totalBanks).getAttribute("textContent");
